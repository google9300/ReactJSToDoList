...
class App extends Component {
  ...
  onEdit = idx => {
    this.setState({ input: this.state.todos[idx], idx: idx });
  };
  onDelete = idx => {
    this.setState({
      todos: this.state.todos.filter((v, index) => index !== idx)
    });
  };
  saveTodo = () => {
        ...
        <ul>
          {todos.map((v, idx) => {
            return (
              <li key={idx}>
                {v}
                <a href="#" onClick={this.onEdit.bind(this, idx)}>
                  Edit
                </a>
                <a href="#" onClick={this.onDelete.bind(this, idx)}>
                  x
                </a>
              </li>
            );
